...
class _MushafDrawerState extends State<MushafDrawer> {
  ...
  List<int> get _juzs => List.generate(30, (i) => i + 1).where((j) {
      ...
      Padding(
          ...
          child: Container(
              ...
              child: TextField(
                  ...
                  style: TextStyle(fontSize: 13, color: isDark ? Colors.white : AppColors.dark),
                  decoration: InputDecoration(
                      hintText: hintText,
                      hintStyle: TextStyle(
                          color: (isDark ? Colors.white : AppColors.dark).withOpacity(0.35), fontSize: 13),
                      ...
                      suffixIcon: _q.isNotEmpty
                          ? IconButton(
                              icon: Icon(Icons.close_rounded,
                                  size: 16,
                                  color: (isDark ? Colors.white : AppColors.dark).withOpacity(0.5)),
                              onPressed: () {
                                _ctrl.clear();
                                setState(() => _q = '');
                              })
  ...
  Widget _list() {
    if (_panel == 1) {
      return _SurahList(
          surahs: _surahs, cur: widget.curSurah, type: _type, onTap: widget.onSurah);
    }
    if (_panel == 2) {
      return _JuzList(
          juzs: _juzs, juzFirstSurah: _juzFirstSurah, onTap: widget.onJuz);
    }
    if (_panel == 3) {
    ...
    return ListView(padding: const EdgeInsets.only(top: 4), children: [
        ...
        Center(
            child: Text('Al-Quran Digital',
                style: TextStyle(
                    fontSize: 11, color: (isDark ? Colors.white : AppColors.dark).withOpacity(0.35)))),
      ]);
  }
}
            ...
            child: Container(
                ...
                child: Row(children: [
                  ...
                  Text(q.getSurahNameArabic(s),
                      style: AppTextStyle.quranSurahNameStyle(
                          fontSize: 18,
                          color: active ? AppColors.gold : textColor.withOpacity(0.75))),
                ])));
      });
  }
}
...
class _JuzList extends StatelessWidget {
  ...
  final Function(int) onTap;
  const _JuzList(
      {required this.juzs, required this.juzFirstSurah, required this.onTap});
  @override
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final textColor = isDark ? Colors.white : AppColors.dark;
        ...
        return InkWell(
            ...
            child: Padding(
                ...
                child: Row(children: [
                  ...
                  Text(q.getSurahNameArabic(s),
                      style: AppTextStyle.quranSurahNameStyle(
                          fontSize: 16, color: textColor.withOpacity(0.65))),
                ])));
      });
  }
}
...
class _PageList extends StatelessWidget {
  ...
  final Function(int) onTap;
  const _PageList(
      {required this.pages, required this.curIdx, required this.onTap});
  String _prev(PageData p) {
    final t = QuranUtils.getCleanVerse(p.verses.first.surah, p.verses.first.verse,
        ...
        return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          if (_newSurah(i))
            Container(
                ...
                decoration: BoxDecoration(
                    color: isDark ? const Color(0xFF252525) : AppColors.dark.withOpacity(0.85),
                    borderRadius:
          ...
          InkWell(
              ...
              child: Container(
                  margin:
                      EdgeInsets.fromLTRB(12, 0, 12, _isLast(i) ? 0 : 1),
                  padding:
                      const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                  decoration: BoxDecoration(
                      color: isCur
                          ? AppColors.gold.withOpacity(0.15)
                          : isDark ? const Color(0xFF1E1E1E) : const Color(0xFFF7F2E8).withOpacity(0.5),
                      border: isCur ? Border.all(color: AppColors.gold, width: 1) : null,
                      borderRadius: _isLast(i)
                  ...
                  child: Row(children: [
                    ...
                    Text('${pg.verses.first.verse}–${pg.verses.last.verse}',
                        style: TextStyle(
                            fontSize: 9, color: textColor.withOpacity(0.4))),
                  ]))),
        ]);
      });
  }
    ...
    return Expanded(
          child: GestureDetector(
              ...
              child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                      color: active ? (isDark ? AppColors.gold : AppColors.dark) : AppColors.gold.withOpacity(0.12),
                      borderRadius: BorderRadius.circular(10),
                      border: Border.all(
                          color: active ? (isDark ? AppColors.gold : AppColors.dark) : AppColors.gold.withOpacity(0.4),
                          width: 1)),
    ...
    return InkWell(
      ...
      child: Padding(
          ...
          child: Row(children: [
            ...
            Text(label, style: TextStyle(fontSize: 14, color: isDark ? Colors.white : AppColors.dark)),
          ])));
  }
}
            ...
            child: Center(
                child: Text('$n',
                    style: TextStyle(
                        fontSize: 10,
                        fontWeight: FontWeight.bold,
                        color: active ? AppColors.gold : textColor.withOpacity(0.7))))));
  }
}
...
class _DiamondBadgePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(
        path,
        Paint()
          ..color = (active ? AppColors.gold.withOpacity(0.15) : AppColors.gold.withOpacity(0.08))
          ..style = PaintingStyle.fill);
    canvas.drawPath(
        path,
        Paint()
          ..color = (active ? AppColors.gold : AppColors.gold.withOpacity(0.5))
...
class _DrawerHeaderPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
        ...
        Paint()
          ..shader = LinearGradient(
                  colors: [AppColors.dark, AppColors.dark.withOpacity(0.82)],
    ...
    canvas.drawLine(Offset(0, h - 1), Offset(w, h - 1),
        Paint()..color = AppColors.gold..strokeWidth = 1.5);
  }
