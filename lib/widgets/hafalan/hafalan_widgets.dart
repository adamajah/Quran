  ...
  Widget build(BuildContext context) {
    ...
    return Wrap(
      ...
      children: words.asMap().entries.map((entry) {
        ...
        final shouldHide = idx % 2 == 1 && !isSymbol && !_revealedWords.contains(idx);
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: shouldHide ? BoxDecoration(
              color: (isDark ? Colors.white : AppColors.dark).withOpacity(0.1),
              borderRadius: BorderRadius.circular(4),
              border: Border.all(color: (isDark ? Colors.white : AppColors.dark).withOpacity(0.2)),
            ) : null,
            child: shouldHide
              ? SizedBox(
                  width: 40, height: widget.fontSize,
                  child: Center(child: Icon(Icons.remove_rounded, size: 12, color: (isDark ? Colors.white : Colors.grey).withOpacity(0.5))))
              : Text(word,
                  style: QuranLibrary().hafsStyle.copyWith(
                    fontSize: widget.fontSize, height: 1.9,
                    color: widget.active ? AppColors.hl : (isDark ? Colors.white : AppColors.ink),
                  ),
                  textDirection: TextDirection.rtl,
                ),
          ),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: active ? AppColors.gold : AppColors.gold.withOpacity(0.35), width: 1.2),
          borderRadius: BorderRadius.circular(12),
        ),
        child: Row(children: [
          Icon(icon, color: active ? AppColors.gold : AppColors.gold, size: 22),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(title, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: textColor)),
            Text(subtitle, style: TextStyle(fontSize: 11, color: textColor.withOpacity(0.55), height: 1.3)),
          ])),
          if (active) Icon(Icons.check_circle_rounded, color: AppColors.gold, size: 18),
        ]),
      ),
    );
  }
    ...
    return Expanded(
      child: GestureDetector(onTap: onTap,
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: active ? AppColors.gold : AppColors.gold.withOpacity(0.10),
            borderRadius: BorderRadius.circular(10),
            border: Border.all(color: active ? AppColors.gold : AppColors.gold.withOpacity(0.35)),
          ),
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(icon, size: 16, color: active ? Colors.white : textColor),
            const SizedBox(height: 3),
            Text(label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: active ? Colors.white : textColor)),
          ]),
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: active ? AppColors.gold.withOpacity(0.12) : (isDark ? Colors.white.withOpacity(0.05) : Colors.grey.shade100),
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: active ? AppColors.gold : (isDark ? Colors.white24 : Colors.grey.shade300)),
        ),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(icon, size: 14, color: active ? AppColors.gold : (isDark ? Colors.white70 : Colors.grey.shade600)),
          const SizedBox(width: 4),
          Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600,
            color: active ? AppColors.gold : (isDark ? Colors.white : Colors.grey.shade700))),
        ]),
      ),
    );
  }
...
class SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(children: [
      ...
      Text(title, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: textColor, letterSpacing: 0.2)),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Text(label, style: TextStyle(fontSize: 10, color: textColor.withOpacity(0.6)))),
        ]),
        const SizedBox(height: 6),
        Text(value, style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: textColor, height: 1)),
        Text(sub, style: TextStyle(fontSize: 10, color: textColor.withOpacity(0.5))),
      ]),
    );
  }
}
    ...
    return SizedBox.square(
      ...
      child: Stack(alignment: Alignment.center, children: [
        ...
        Text('${(value * 100).round()}%',
          style: TextStyle(fontSize: size * 0.22, fontWeight: FontWeight.w800, color: textColor)),
      ]),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(gradient: LinearGradient(colors: [
        AppColors.gold.withOpacity(0), AppColors.gold, isDark ? Colors.white : AppColors.dark, AppColors.gold, AppColors.gold.withOpacity(0),
      ])),
    );
  }
}
...
BoxDecoration cardDeco(BuildContext context) {
  ...
  return BoxDecoration(
    ...
    boxShadow: [BoxShadow(color: (isDark ? Colors.black : AppColors.dark).withOpacity(0.07), blurRadius: 8, offset: const Offset(0, 2))],
  );
}
